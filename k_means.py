...
def optimize_splitters_with_kmeans(ONU_positions, num_splitters):
    ...
    splitters_positions = list(np.round(kmeans.cluster_centers_).astype(int))
    ...
    splitters_positions = [tuple(splitter) if isinstance(splitter, np.ndarray) else splitter for splitter in splitters_positions]

    ONU_positions = assign_ONU_to_splitters(splitters_positions, ONU_positions)
    

    total_distance,mst = calculate_total_length(splitters_positions, ONU_positions)
...
def main():
    ...
    for num_splitters in range(1, 2+int(len(ONU_positions)**0.5)):
        splitters,onus,dist,mst = optimize_splitters_with_kmeans(ONU_positions, num_splitters)
        print(dist)
